...
export default function BillingSuccess() {
  ...
  useEffect(() => {
    ...
    // If no session_id, this might be from an Apple IAP purchase
    // In that case, just refresh the plan and show success
    const verifySubscription = async () => {
      setVerifying(true);
      try {
        // Only call sync-subscription if we have a Stripe session_id
        if (sessionId) {
          // Legacy Stripe flow - sync from Stripe
          await supabase.functions.invoke('sync-subscription');
        }
      } catch (e) {
        console.warn('sync-subscription failed (will still refresh profile):', e);
      }
      // Refresh local plan data regardless
      await refresh();
      setVerifying(false);
    };
